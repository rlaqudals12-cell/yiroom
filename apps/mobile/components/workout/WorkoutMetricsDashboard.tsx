...
export function WorkoutMetricsDashboard({
  ...
  return (
    <View testID="workout-metrics-dashboard" accessibilityLabel={`${periodLabel} 운동 통계`}>
      <Text
        style={{
      ...
      <View style={styles.metricsGrid}>
        {[
          ...
          >
            ...
            <Text style={{ color: baseColor, fontSize: typography.size.lg, fontWeight: '700' }}>
              {item.value}
            </Text>
        ...
        <View style={{ marginTop: spacing.sm }}>
          ...
          {metrics.personalBests.slice(0, 3).map((pb, i) => (
            <View key={i} style={styles.pbRow}>
              ...
              <Text style={{ color: baseColor, fontSize: typography.size.xs, fontWeight: '600' }}>
                {pb.record}
              </Text>
            </View>
          ))}
