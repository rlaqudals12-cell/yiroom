...
describe('GradientCard', () => {
  ...
  describe('다크 모드', () => {
    ...
    it('다크 모드에서 보더 투명도가 0.2로 적용되어야 한다', () => {
      // brand variant: glowColor = '#C56A84' → rgba(197,106,132,0.2)
      const { getByTestId } = renderWithTheme(
      ...
      const hasBorderColor = flatStyle.some(
        (s: Record<string, unknown>) => s && s.borderColor === 'rgba(197,106,132,0.2)'
      );
      expect(hasBorderColor).toBe(true);
    });

    it('라이트 모드에서 보더 투명도가 0.15로 적용되어야 한다', () => {
      // brand variant: glowColor = '#C56A84' → rgba(197,106,132,0.15)
      const { getByTestId } = renderWithTheme(
      ...
      const flatStyle = Array.isArray(card.props.style)
        ...
        : [card.props.style];
      const hasBorderColor = flatStyle.some(
        (s: Record<string, unknown>) => s && s.borderColor === 'rgba(197,106,132,0.15)'
      );
      expect(hasBorderColor).toBe(true);
    });
  });
