...
describe('AnalysisLoadingState', () => {
  describe('기본 렌더링', () => {
    it('기본 testID "analysis-loading"이 존재해야 한다', () => {
      const { getByTestId } = render(<AnalysisLoadingState />);
    ...
    it('기본 로딩 메시지가 표시되어야 한다', () => {
      const { getByText } = render(<AnalysisLoadingState />);
  ...
  describe('ActivityIndicator', () => {
    it('로딩 인디케이터가 렌더링되어야 한다', () => {
      const { UNSAFE_getByType } = render(<AnalysisLoadingState />);
      const { ActivityIndicator } = require('react-native');
      const indicator = UNSAFE_getByType(ActivityIndicator);
      expect(indicator).toBeTruthy();
    });

    it('로딩 인디케이터 색상이 #2e5afa여야 한다', () => {
      const { UNSAFE_getByType } = render(<AnalysisLoadingState />);
      const { ActivityIndicator } = require('react-native');
      const indicator = UNSAFE_getByType(ActivityIndicator);
      expect(indicator.props.color).toBe('#2e5afa');
    });
  ...
  describe('다크 모드', () => {
    it('isDark=false에서 정상 렌더링되어야 한다', () => {
      const { getByTestId, getByText } = render(
        <AnalysisLoadingState isDark={false} />
      );
      expect(getByTestId('analysis-loading')).toBeTruthy();
      expect(getByText('분석 중이에요...')).toBeTruthy();
    });
    ...
    it('isDark=true에서도 정상 렌더링되어야 한다', () => {
      const { getByTestId, getByText } = render(
        <AnalysisLoadingState isDark={true} />
      );
      expect(getByTestId('analysis-loading')).toBeTruthy();
      expect(getByText('분석 중이에요...')).toBeTruthy();
    });
    ...
    it('다크 모드에서 커스텀 메시지가 표시되어야 한다', () => {
      const { getByText } = render(
        <AnalysisLoadingState isDark={true} message="결과를 준비 중..." />
      );
