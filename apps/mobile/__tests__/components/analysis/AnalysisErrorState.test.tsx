...
describe('AnalysisErrorState', () => {
  describe('기본 렌더링', () => {
    it('기본 testID "analysis-error"가 존재해야 한다', () => {
      const { getByTestId } = renderWithTheme(<AnalysisErrorState />);
    ...
    it('기본 에러 메시지가 표시되어야 한다', () => {
      const { getByText } = renderWithTheme(<AnalysisErrorState />);
  ...
  describe('재시도 버튼', () => {
    it('onRetry가 없으면 재시도 버튼이 표시되지 않아야 한다', () => {
      const { queryByTestId } = renderWithTheme(<AnalysisErrorState />);
