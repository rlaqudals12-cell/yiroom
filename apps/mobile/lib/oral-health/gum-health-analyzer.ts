 ...
 *
 * @param input - 분석 입력
 * @param useApi - 웹 API 사용 여부 (기본: true)
 * @returns 잇몸 건강 분석 결과
 */
 ...
 *
 * @description
 * 웹 API를 통해 분석 결과를 기반으로 Lab 픽셀 분포 생성.
 * API 사용 불가 시 Mock 데이터로 폴백.
 *
 * @param imageBase64 - Base64 인코딩된 이미지
 * @returns Lab 색상 배열
 */
async function extractGumPixelsFromImage(imageBase64: string): Promise<LabColor[]> {
  // API 브릿지를 통한 픽셀 추출 시도
  const apiResult = await extractGumPixelsWithApi(imageBase64);

  if (apiResult && !apiResult.usedFallback) {
    return apiResult.pixels;
  }
