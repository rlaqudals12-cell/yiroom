...
export function getNextMilestone(
  ...
  const milestones = ALL_MILESTONES.filter((m) => m.category === category).sort(
    (a, b) => a.threshold - b.threshold
  );

  return milestones.find((m) => !achievedIds.includes(m.id) && currentValue < m.threshold) ?? null;
}
...
export function checkNewMilestones(
  ...
  return ALL_MILESTONES.filter(
    (m) => m.category === category && currentValue >= m.threshold && !achievedIds.includes(m.id)
  );
}
