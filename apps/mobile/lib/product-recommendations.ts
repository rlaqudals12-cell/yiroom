...
const CARE_TIPS_BY_TYPE: Record<SkinType, CareTips> = {
  ...
  oily: {
    weeklyCare: ['주 1-2회 클레이 마스크', '주 2회 BHA 각질 케어', '주 1회 모공 팩'],
    lifestyleTips: ['기름진 음식 줄이기', '충분한 수분 섭취', '세안 후 즉시 보습', '스트레스 관리'],
  },
  sensitive: {
    weeklyCare: ['주 1회 진정 마스크팩', '자극 성분 패치 테스트 필수', '새 제품은 소량부터 테스트'],
    lifestyleTips: [
      '자극적인 음식 피하기',
      '온도 변화 최소화',
      '손으로 얼굴 만지지 않기',
  ...
  combination: {
    weeklyCare: ['주 1회 T존 클레이 마스크', '주 1-2회 U존 수분 마스크', '주 1회 가벼운 각질 케어'],
    lifestyleTips: ['부위별 맞춤 케어', '균형 잡힌 식단', '물 충분히 섭취', '규칙적인 수면'],
  },
  normal: {
    weeklyCare: ['주 1-2회 각질 케어', '주 2-3회 시트 마스크', '주 1회 영양 마스크'],
    lifestyleTips: ['물 2L 이상 섭취', '7시간 이상 수면', '자외선 차단 철저히', '현재 루틴 유지'],
  },
};
 ...
 * @returns 추천 제품 목록
 */
export function getProductsForConcerns(concerns: string[], skinType: SkinType): Product[] {
 ...
 * @returns 메이크업 추천 목록
 */
export function getMakeupRecommendations(season: string | null): MakeupRecommendation[] {
 ...
 */
export function extractConcernsFromMetrics(metrics: Record<string, number | null>): string[] {
...
function sortByPriceRange(products: Product[]): Product[] {
  return [...products].sort((a, b) => PRICE_ORDER[a.priceRange] - PRICE_ORDER[b.priceRange]);
}
...
export function generateProductRecommendations(
  ...
  // 5. 스킨케어 루틴 (아침/저녁)
  const skincareRoutine = SKINCARE_ROUTINE_BY_TYPE[skinType] || SKINCARE_ROUTINE_BY_TYPE.normal;
