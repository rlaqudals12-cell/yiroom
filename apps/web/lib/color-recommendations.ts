...
/**
 * 퍼스널컬러 시즌 원시값 → 사용자 대면 한국어 라벨 ("Spring" → "봄 웜톤").
 *
 * DB에는 대문자('Spring')와 소문자('spring')가 섞여 있어 양쪽을 모두 받는다.
 * 계산·저장은 원시 코드값 그대로 두고, **문구를 만드는 시점에만** 이 함수를 통과시킨다
 * (스코어링·매칭 로직은 불변 — 라벨은 표시 전용).
 *
 * 매칭 실패 시 '기타' — 원시 영문값이 사용자 화면에 새어나가지 않게 한다.
 */
const PERSONAL_COLOR_SEASON_LABELS: Record<string, string> = {
  spring: '봄 웜톤',
  summer: '여름 쿨톤',
  autumn: '가을 웜톤',
  winter: '겨울 쿨톤',
...
export function getPersonalColorSeasonLabel(season: string | null | undefined): string {
  if (!season) return '기타';
  return PERSONAL_COLOR_SEASON_LABELS[season.toLowerCase()] ?? '기타';
}
 ...
 */
const BODY_TYPE_COLOR_STRATEGY: Record<
  BodyType,
  {
 ...
 * 3타입 체형별 권장 색상 배치 전략
 */
const BODY_TYPE_3_COLOR_STRATEGY: Record<
  BodyType3,
  {
    topPriority: ColorBrightnessPriority;
    bottomPriority: ColorBrightnessPriority;
...
> = {
  S: {
    topPriority: 'balanced',
    bottomPriority: 'balanced',
    tips: ['I라인 실루엣을 위해 상하의 톤을 맞춰보세요', '단색 코디가 깔끔해요'],
  },
  W: {
    topPriority: 'light',
    bottomPriority: 'dark',
    tips: ['상의는 밝게, 하의는 어둡게 해서 X라인을 강조해요', '허리에 포인트 컬러를 줘보세요'],
  },
  N: {
    topPriority: 'balanced',
    bottomPriority: 'balanced',
    tips: ['자연스러운 어스톤 계열이 잘 어울려요', '레이어드로 색상을 다양하게 활용해보세요'],
  },
};
 ...
 * 밝기에 따라 색상 필터링
 */
function filterColorsByBrightness(colors: string[], priority: ColorBrightnessPriority): string[] {
  // 밝은 색상 키워드
  ...
  const isLight = (color: string) => lightKeywords.some((k) => color.includes(k));
  const isDark = (color: string) => darkKeywords.some((k) => color.includes(k));
 ...
 * 최적의 색상 조합 생성
 */
function generateBestCombinations(topColors: string[], bottomColors: string[]): ColorCombination[] {
...
export function generateColorRecommendations(
  ...
  const strategy = bodyType ? BODY_TYPE_COLOR_STRATEGY[bodyType as BodyType] : null;
