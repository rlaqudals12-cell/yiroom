 ...
 * H-1 얼굴형/헤어 분석 프롬프트
 */
const HAIR_V2_PROMPT = `당신은 전문 헤어 스타일리스트이자 얼굴형 분석 전문가입니다. 업로드된 얼굴 이미지에서 얼굴형, 모발 텍스처, 손상도, 두피 상태를 종합 분석해주세요.

📋 분석 순서 (Step-by-Step):
1. 이미지 품질 평가 (얼굴 전체 보임 여부, 정면/측면)
2. 얼굴 비율 측정 (길이, 너비, 이마, 광대, 턱)
3. 얼굴형 유형 판정
4. 모발 텍스처 분석 (Andre Walker 12-Type)
5. 모발 손상도/두피 상태 추정
6. 어울리는 헤어스타일 추천
7. 피해야 할 스타일 안내
...
💇 모발 텍스처 (Andre Walker 12-Type):
- Type 1 (직모): 1a(매우 가는), 1b(중간), 1c(굵은)
- Type 2 (웨이브): 2a(가벼운), 2b(중간), 2c(강한)
- Type 3 (곱슬): 3a(느슨한), 3b(스프링), 3c(타이트)
- Type 4 (코일리): 4a(소프트), 4b(Z형), 4c(타이트)
...
- 모발이 보이지 않으면 hairAnalysis 필드는 기본값 사용
...

{
  ...
  "hairAnalysis": {
    ...
    "textureCode": "1a|1b|1c|2a|2b|2c|3a|3b|3c|4a|4b|4c",
    "thickness": "fine|medium|thick",
    "damageLevel": 0-100,
    "damageIndicators": ["split ends", "breakage", "dryness", "color damage", "heat damage", "none"],
    "scalpCondition": "dry|normal|oily|sensitive",
    "hairDensity": "thin|normal|dense",
    "curlIntensity": 0-100,
...
const GeminiHairV2ResponseSchema = z.object({
  ...
  // 모발 텍스처/손상/두피 상세 분석 (v2 확장)
  hairAnalysis: z
    .object({
      textureType: z.enum(['straight', 'wavy', 'curly', 'coily']),
      textureCode: z.enum(['1a', '1b', '1c', '2a', '2b', '2c', '3a', '3b', '3c', '4a', '4b', '4c']),
      thickness: z.enum(['fine', 'medium', 'thick']),
      damageLevel: z.number().min(0).max(100),
      damageIndicators: z.array(z.string()),
      scalpCondition: z.enum(['dry', 'normal', 'oily', 'sensitive']),
      hairDensity: z.enum(['thin', 'normal', 'dense']),
      curlIntensity: z.number().min(0).max(100),
