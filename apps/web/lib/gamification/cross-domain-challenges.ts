/**
 * 크로스도메인 챌린지 시스템
 * @description 뷰티+운동+영양을 조합한 멀티도메인 챌린지 정의 및 진행 추적
 */

import { calculateChallengeXp } from './challenges';
import type { ChallengeDefinition, ChallengeDomain } from './challenges';
...
export interface DomainRequirement {
  domain: Exclude<ChallengeDomain, 'cross'>;
  /** 필요 횟수 */
  count: number;
  /** 활동 라벨 (UI 표시용) */
  label: string;
...
/** 크로스도메인 챌린지 정의 (기본 + 도메인별 조건) */
export interface CrossDomainChallengeDefinition extends ChallengeDefinition {
  domain: 'cross';
  /** 도메인별 세부 요구 조건 */
...
export const CROSS_DOMAIN_CHALLENGES: CrossDomainChallengeDefinition[] = [
  // 입문: 각 도메인 1회씩
  {
    ...
    difficulty: 'easy',
    targetCount: 3,
    xpReward: calculateChallengeXp('easy', 75),
    minLevel: 1,
    durationDays: 7,
    requirements: [
  ...
  // 주간 균형 챌린지
  {
    ...
    difficulty: 'medium',
    targetCount: 11,
    xpReward: calculateChallengeXp('medium', 100),
    minLevel: 3,
    durationDays: 7,
    requirements: [
  ...
  // 종합 마스터 (hard)
  {
    ...
    difficulty: 'hard',
    targetCount: 41,
    xpReward: calculateChallengeXp('hard', 150),
    minLevel: 5,
    durationDays: 30,
    requirements: [
...
export function calculateCrossDomainProgress(
  ...
  activityCounts: Partial<Record<Exclude<ChallengeDomain, 'cross'>, number>>
): DomainProgress[] {
...
const DOMAIN_LABELS: Record<Exclude<ChallengeDomain, 'cross'>, string> = {
  beauty: '뷰티',
  workout: '운동',
  nutrition: '식단',
  wellness: '웰니스',
