/**
 * 제품 시너지/충돌 탐지
 * @description 인벤토리 내 스킨케어 성분 시너지 및 충돌 분석
 */
...
/**
 * 인벤토리 내 모든 뷰티 제품 간 상호작용 분석
 * @param products - [{name, tags}] 형태의 제품 목록
 * @returns 모든 상호작용 (중복 제거)
 */
export function analyzeInventoryInteractions(
  products: Array<{ name: string; tags?: string[] }>
): Array<ProductInteraction & { productA: string; productB: string }> {
  const results: Array<ProductInteraction & { productA: string; productB: string }> = [];
 ...
 * 스킨케어 제품 도포 순서 추천
 */
export function suggestRoutineOrder(products: Array<{ name: string }>): RoutineOrderSuggestion[] {
