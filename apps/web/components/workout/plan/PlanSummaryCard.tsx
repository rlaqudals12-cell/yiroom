      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        ...
        <div className="bg-indigo-50 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm text-muted-foreground">{t('planSummaryCard0')}</span>
          </div>
        ...
        <div className="bg-muted rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-sm text-muted-foreground">{t('planSummaryCard2')}</span>
          </div>
        ...
        <div className="bg-blue-50 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm text-muted-foreground">{t('planSummaryCard3')}</span>
          </div>
          <p className="text-2xl font-bold text-blue-600">
            {totalMinutes}
            <span className="text-sm font-normal text-muted-foreground">분</span>
          </p>
          <p className="text-xs text-muted-foreground mt-1">하루 평균 {avgMinutesPerDay}분</p>
        </div>
        ...
        <div className="bg-orange-50 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm text-muted-foreground">{t('planSummaryCard4')}</span>
          </div>
          <p className="text-2xl font-bold text-orange-600">
            {totalCalories.toLocaleString()}
            <span className="text-sm font-normal text-muted-foreground">kcal</span>
          </p>
          <p className="text-xs text-muted-foreground mt-1">하루 평균 {avgCaloriesPerDay}kcal</p>
        </div>
      </div>

      {/* 부위 분포 */}
      <div>
        <h4 className="text-sm font-medium text-foreground mb-3">{t('planSummaryCard5')}</h4>
            ...
            .map(([category, ratio]) => {
              ...
              return (
                <div key={category} className="flex items-center gap-3">
                  <span className={`text-xs font-medium ${info.color} w-12`}>{info.label}</span>
