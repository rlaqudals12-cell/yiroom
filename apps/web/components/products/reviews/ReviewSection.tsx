...
export function ReviewSection({
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      const [summaryData, reviewsData] = await Promise.all([
        ...
        getProductReviews(productType, productId, {
  ...
  const handleSortChange = async (newSort: ReviewSortBy) => {
    ...
    setIsLoading(true);
    try {
      const reviewsData = await getProductReviews(productType, productId, {
        limit: REVIEWS_PER_PAGE,
        sortBy: newSort,
      }, user?.id);
      setReviews(reviewsData);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const moreReviews = await getProductReviews(productType, productId, {
        limit: REVIEWS_PER_PAGE,
    ...
    <Card className={cn(className)} data-testid="review-section">
      ...
      <CardContent className="space-y-6">
        ...
        {!isLoading && user && !userHasReviewed && (
          <ReviewPromptCard onWrite={handleWriteClick} />
        )}
