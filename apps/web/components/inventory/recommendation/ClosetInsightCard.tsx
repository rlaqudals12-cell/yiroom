...
export function ClosetInsightCard({
  ...
  const closetCount = items.filter((item) => item.category === 'closet').length;
  ...
  return (
    <div
      ...
      className={`bg-card rounded-xl border p-4 ${className}`}
    >
      {/* 헤더 */}
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`font-semibold ${getStatusColor(matchRate)}`}>
            {matchRate}%
          </span>
        </div>
      ...
      {summary.suggestions.length > 0 && (
        <div className="bg-muted/50 rounded-lg p-3">
          <div className="flex items-start gap-2">
            <StatusIcon
              className={`w-4 h-4 mt-0.5 ${getStatusColor(matchRate)}`}
            />
