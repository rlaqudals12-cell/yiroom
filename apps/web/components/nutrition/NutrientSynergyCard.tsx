...
// 상호작용 타입별 스타일
const INTERACTION_STYLES: Record<
  InteractionType,
  { icon: typeof ArrowUp; color: string; bg: string; label: string }
> = {
  synergy: {
    icon: ArrowUp,
    ...
    <div className={cn('rounded-lg overflow-hidden border', style.bg)}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-foreground">{NUTRIENT_LABELS[nutrient2] || nutrient2}</p>
...
export function NutrientSynergyCard({
  ...
  const antagonistNutrients = useMemo(
    () => getAntagonistNutrients(activeNutrient),
    [activeNutrient]
  );

  // 사용 가능한 영양소 목록
  const availableNutrients = useMemo(() => Object.keys(NUTRIENT_INTERACTION_MATRIX), []);
  ...
  return (
    <div
      className={cn('bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden', className)}
    ...
    >
      ...
      <div className="px-5 pb-4">
        <p className="text-xs text-muted-foreground text-center">
          ※ 참고용 정보예요. 개인별 건강 상태에 따라 다를 수 있어요.
        </p>
      </div>
    </div>
  );
