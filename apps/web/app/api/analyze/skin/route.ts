...
export async function POST(req: NextRequest) {
  try {
    ...
    // 분석 신뢰도 결정
    const analysisReliability = selectByKey(
      imagesCount,
      { 3: 'high' as const, 2: 'medium' as const },
      'low' as const
    )! satisfies AnalysisReliabilityLevel;
    ...
    try {
      ...
      // 피부 타입 결정 (T존/U존 분리 + 민감도, Sebumeter 기준)
      // 근거: docs/principles/skin-physiology.md §2.3
      const oilLevel = getMetricValue('oil') || 50;
      const hydrationLevel = getMetricValue('hydration') || 50;
      const sensitivityLevel = getMetricValue('sensitivity', 'trouble') || 50;

      // T존(유분)과 U존(수분) 차이로 복합성 판정
      const tZoneOily = oilLevel >= 60;
      const uZoneDry = hydrationLevel <= 45;
      const tUzoneDiff = oilLevel - (100 - hydrationLevel);
      ...
      if (sensitivityLevel >= 70) {
        ...
        // T존 유분 높고 U존 건조 → 복합성 (Sebumeter T>150, U<30 패턴)
        skinType = 'combination';
      } else if (oilLevel >= 60 && hydrationLevel >= 55) {
        // 전체적으로 유분 높고 수분도 보통 이상 → 지성
        skinType = 'oily';
      } else if (oilLevel <= 40 && hydrationLevel <= 45) {
        // 유분 낮고 수분도 낮음 → 건성
        skinType = 'dry';
      } else if (Math.abs(tUzoneDiff) > 25) {
        // T존/U존 차이가 극단적 → 복합성
        skinType = 'combination';
      }
