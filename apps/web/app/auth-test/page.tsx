...
export default function AuthTestPage() {
  ...
  const testConnection = useCallback(async () => {
    try {
      ...
      if (error) {
        // Supabase 에러 객체 처리
        throw new Error(error.message || "Supabase 연결 실패");
      }
      ...
      const errorMessage =
        err instanceof Error ? err.message : "연결 테스트 실패";
      setError(errorMessage);
      console.error("Connection test error:", errorMessage);
    }
  }, [supabase]);

  // Supabase 에러 메시지 추출 헬퍼
  ...
  const fetchOrCreateUser = useCallback(async () => {
    ...
    try {
      ...
      if (fetchError && fetchError.code !== "PGRST116") {
        throw new Error(fetchError.message || "사용자 조회 실패");
      }
      ...
      setError(errorMessage);
      console.error("Fetch or create user error:", errorMessage);
    } finally {
      setLoading(false);
    }
  }, [user, supabase]);
