...
interface GalleryMultiAngleSkinUploadProps {
  /** 업로드 완료 핸들러 */
...
const ANGLE_DESCRIPTIONS: Record<FaceAngle, string> = {
  front: '정면 피부 상태를 볼 수 있는 사진',
  left: '왼쪽 볼/광대 피부를 볼 수 있는 사진',
  right: '오른쪽 볼/광대 피부를 볼 수 있는 사진',
};

export default function GalleryMultiAngleSkinUpload({
  onComplete,
  onValidate,
  onCancel,
}: GalleryMultiAngleSkinUploadProps) {
  // 선택된 이미지 (Base64)
          ...
          try {
            ...
            if (result.quality.makeupDetected) {
              setValidationWarning(
                '메이크업이 감지되었어요. 맨 얼굴 사진이 피부 분석에 더 정확해요.'
              );
            }
            ...
            if (result.quality.lighting === 'dark' || result.quality.lighting === 'uneven') {
              setValidationWarning(
                '조명이 어둡거나 고르지 않아요. 자연광에서 찍은 사진이 피부 분석에 더 좋아요.'
              );
            }
          } catch (validationError) {
            console.error('[GallerySkinUpload] Validation error:', validationError);
            // 검증 실패해도 이미지 업로드는 허용
          } finally {
            setIsValidating(false);
          }
    ...
    <div className="space-y-6" data-testid="gallery-multi-angle-skin-upload">
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          좌/우 볼 피부까지 분석하면 더 정확한 결과를 받을 수 있어요
        </p>
      ...
      {validationWarning && !error && (
        <div className="p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg text-amber-700 dark:text-amber-400 text-sm flex items-start gap-2">
