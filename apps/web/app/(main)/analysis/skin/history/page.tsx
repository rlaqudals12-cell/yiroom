...
export default function SkinHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/analysis/history?type=skin&period=${period}&limit=20`);
        if (res.ok) {
          const data: AnalysisHistoryResponse = await res.json();
          setAnalyses(data.analyses as SkinAnalysisHistoryItem[]);
          setTrend(data.trend);
        }
      } catch (error) {
        console.error('[Skin History] Fetch error:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCompare = () => {
    if (selectedIds.length === 2) {
      router.push(`/analysis/skin/compare?from=${selectedIds[0]}&to=${selectedIds[1]}`);
    }
  };
      ...
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">피부 변화 기록</h1>
          </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">
                          {skinTypeLabel} | 점수: {item.overallScore}점
                        </p>
                        <div className="flex gap-3 mt-2">
                          <div className="flex items-center gap-1 text-xs text-blue-600">
                            <Droplet className="h-3 w-3" aria-hidden="true" />
                            <span>수분 {item.details.hydration}</span>
                          </div>
                          <div className="flex items-center gap-1 text-xs text-yellow-600">
                            <Sun className="h-3 w-3" aria-hidden="true" />
                            <span>유분 {item.details.oilLevel}</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {isSelected && (
                      <div className="mt-2 pt-2 border-t">
                        <p className="text-xs text-pink-600">✓ 비교 대상으로 선택됨</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
