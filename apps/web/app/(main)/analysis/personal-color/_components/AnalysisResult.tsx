    ...
    <div className="space-y-6" data-testid="analysis-result">
      ...
      <ScaleIn>
        ...
        >
          ...
          {/* 서브톤 표시 */}
          {undertoneLabel && <p className="text-sm text-muted-foreground mt-1">{undertoneLabel}</p>}
        ...
        <section className="bg-card rounded-xl border p-6">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
            {bestColors.map((color, index) => (
              ...
              >
                <div
                  className="w-full aspect-square rounded-lg shadow-sm border border-border group-hover:ring-2 group-hover:ring-primary/50 transition-all"
                  style={{ backgroundColor: color.hex }}
                />
                <p className="text-xs text-muted-foreground mt-1 truncate">
                  {getKoreanColorName(color.hex)}
                </p>
              </button>
            ))}
          </div>
          {/* "왜 이 색이 어울리는지" 1줄 요약 (E2 L2 가치 — 상세는 리포트 탭에서) */}
          {SEASON_WHY[seasonType] && (
            <p className="mt-3 text-xs text-muted-foreground">
              ...
              <button
                type="button"
                className="ml-1 text-primary/70 hover:text-primary underline-offset-2 hover:underline cursor-pointer"
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {bestColors[0] ? getKoreanColorName(bestColors[0].hex) : ''}
                </p>
              ...
              <div className="text-center">
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">
                  {worstColors[0] ? getKoreanColorName(worstColors[0].hex) : ''}
                </p>
                ...
                <div className="space-y-2">
                  <p className="text-sm text-foreground/80">
                    <span className="font-medium">
                      {genderStyleDescription.easyAccessory.metal}
                    </span>
                    이 잘 어울려요
                  </p>
                  <div className="flex flex-wrap gap-1">
