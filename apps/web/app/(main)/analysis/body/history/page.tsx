...
export default function BodyHistoryPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/analysis/history?type=body&period=${period}&limit=20`);
        if (res.ok) {
          const data: AnalysisHistoryResponse = await res.json();
          setAnalyses(data.analyses as BodyAnalysisHistoryItem[]);
          setTrend(data.trend);
        }
      } catch (error) {
        console.error('[Body History] Fetch error:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCompare = () => {
    if (selectedIds.length === 2) {
      router.push(`/analysis/body/compare?from=${selectedIds[0]}&to=${selectedIds[1]}`);
    }
  };
      ...
      <header className="sticky top-0 z-40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">체형 변화 기록</h1>
          </div>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-4">
                      ...
                      {item.imageUrl ? (
                        <div
                          ...
                          role="img"
                          aria-label="체형 분석 이미지"
                        />
                      ) : (
                        <div className="w-16 h-16 rounded-lg bg-muted flex items-center justify-center">
                          <span className="text-2xl">{item.details.bodyType}</span>
                        </div>
                      )}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">
                          체형: {item.details.bodyType} | 점수: {item.overallScore}점
                        </p>
                        <div className="flex gap-2 mt-1 text-xs text-muted-foreground">
                          <span>어깨 {item.details.shoulder}</span>
                          <span>허리 {item.details.waist}</span>
                          <span>힙 {item.details.hip}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    {isSelected && (
                      <div className="mt-2 pt-2 border-t">
                        <p className="text-xs text-primary">✓ 비교 대상으로 선택됨</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
