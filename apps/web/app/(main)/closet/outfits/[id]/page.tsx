...
import { ArrowLeft, Edit2, Trash2, Calendar, Tag, Shirt, TrendingUp, Sun } from 'lucide-react';
  ...
  const loadOutfit = useCallback(async () => {
    ...
    if (outfitData.item_ids?.length > 0) {
      ...
      if (!itemsError && itemsData) {
        const rows = itemsData as InventoryItemDB[];
        // 비공개 버킷 — 코디에 포함된 아이템 이미지를 한 번에 서명한다(아이템별 서명 = N+1 요청)
  ...
  return (
    <div data-testid="outfit-detail-page" className="min-h-screen pb-20">
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>이 코디를 삭제하시겠습니까?</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
      ...
      <div className="px-4 py-4 space-y-6">
        {/* 기본 정보 */}
        <div>
          <h1 className="text-2xl font-bold">{outfit.name || '이름 없는 코디'}</h1>
          {outfit.description && <p className="text-muted-foreground mt-1">{outfit.description}</p>}
        </div>
        ...
        <div className="space-y-4">
          ...
          {outfit.occasion && (
            <div className="flex items-start gap-3">
              ...
              <Badge variant="secondary">{OCCASION_LABELS[outfit.occasion as Occasion]}</Badge>
            </div>
          )}
        </div>
        ...
        <div className="text-xs text-muted-foreground pt-4 border-t">
          등록일: {format(new Date(outfit.created_at), 'yyyy년 M월 d일', { locale: ko })}
        </div>
      </div>
    </div>
  );
