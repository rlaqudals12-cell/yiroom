/**
 * 공유카드용 12톤 큐레이션 팔레트 — 진단 톤의 "표준 스와치" (웹·모바일 공유)
 * (베스트 6 + 피해야 할 색 4 + 포인트 컬러 3 + 액세서리 금속 2)
 *
 * 2026-07-23 apps/web/lib/share/tone-palettes.ts에서 이관 — 모바일 E+ 카드 포팅이
 * 동일 큐레이션을 소비해야 해서(웹-모바일 색 드리프트 방지) 공유 패키지로 승격.
 * 웹은 기존 경로를 재수출 심으로 유지한다.
 *
 ...
 *
 * @module tone-palettes
 * @see memory design-taste-moat(진단 팔레트=주인공) · 조사 2026-07-15(업계 팔레트 관습)
 */

/** 카드 산출물 로케일 (웹 OutputLocale과 동일 union — 플랫폼 의존 없이 재정의) */
export type CardLocale = 'ko' | 'en' | 'ja' | 'zh';
...
/**
 * 12톤 ko 라벨 — 모바일 카드 히어로용.
 * ⚠️ 정본(SSOT)은 웹 `apps/web/lib/analysis/personal-color-v2/types.ts`의 TWELVE_TONE_LABELS.
 * 웹 테스트(tone-palettes.test.ts)가 두 테이블의 동일성을 강제해 드리프트를 막는다.
 */
export const TONE_LABELS_KO: Record<string, string> = {
...
/** 4계절 ko 라벨 — tone 부재 시 폴백 히어로 */
export const SEASON_LABELS_KO: Record<string, string> = {
  spring: '봄 웜톤',
  summer: '여름 쿨톤',
  autumn: '가을 웜톤',
  winter: '겨울 쿨톤',
...
/** 진단 톤/계절 → ko 히어로 라벨 (미지 값이면 null — 원시 영문키를 카드에 올리지 않는다) */
export function toneHeroLabelKo(
  tone: string | null | undefined,
  season: string | null | undefined
): string | null {
  if (tone && TONE_LABELS_KO[tone]) return TONE_LABELS_KO[tone];
  if (season && SEASON_LABELS_KO[season]) return SEASON_LABELS_KO[season];
  return null;
}
