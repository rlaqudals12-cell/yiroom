...
### 2.1 사진 기반 오버레이 (Phase 2)

| 항목            | 내용                                         |
| --------------- | -------------------------------------------- |
| **스펙**        | SDD-VISUAL-SKIN-REPORT.md §5.2               |
| **기능**        | 사용자 얼굴 사진 위에 피부 존 오버레이       |
| **보류 사유**   | 얼굴 랜드마크 정합 복잡, MediaPipe 통합 필요 |
| **의존성**      | 이미지 동의 플로우 (Phase 1 완료 필요)       |
| **예상 복잡도** | 85점 (full 전략)                             |
| **우선순위**    | 중간                                         |

### 2.2 Before/After 슬라이더

| 항목          | 내용                              |
| ------------- | --------------------------------- |
| **스펙**      | SDD-VISUAL-SKIN-REPORT.md §5.2    |
| **기능**      | 이전/현재 피부 사진 비교 슬라이더 |
| **보류 사유** | Phase 2 의존 (사진 저장 필요)     |
| **컴포넌트**  | `BeforeAfterSlider.tsx` (미구현)  |
| **우선순위**  | 중간                              |
...
| **스펙**      | SDD-VISUAL-SKIN-REPORT.md §5.2 |
| **기능**      | 피부 점수 변화 월별 그래프     |
| **보류 사유** | 데이터 축적 필요 (최소 3개월)  |
| **컴포넌트**  | `TrendChart.tsx` (미구현)      |
| **우선순위**  | 낮음                           |
...
### 4.3 AI Fallback 패턴 불일치

| 항목          | 내용                                           |
| ------------- | ---------------------------------------------- |
| **모범 사례** | `lib/products/services/ingredient-analysis.ts` |
| **개선 필요** | `lib/gemini.ts` 일부 함수 Mock Fallback 누락   |
| **해결 방안** | 타임아웃/재시도 일관성 강화                    |
| **우선순위**  | 중간                                           |
