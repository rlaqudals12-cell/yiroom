...
> **업데이트**: 캡슐 에코시스템 5 Phase 구현 완료 (60+ 파일, 204 테스트)
...
### 4.5 캡슐 에코시스템 (신규 2026-03-04)

> **ADR**: [ADR-069](./adr/ADR-069-capsule-ecosystem-architecture.md), [ADR-070](./adr/ADR-070-safety-profile-architecture.md), [ADR-071](./adr/ADR-071-cross-module-scoring.md), [ADR-073](./adr/ADR-073-one-button-daily.md)
> **스펙**: [SDD-CAPSULE-ECOSYSTEM](./specs/SDD-CAPSULE-ECOSYSTEM.md), [SDD-SAFETY-PROFILE](./specs/SDD-SAFETY-PROFILE.md)

| Phase                  | 내용                                  | 파일 수 | 테스트                           | 상태            |
| ---------------------- | ------------------------------------- | ------- | -------------------------------- | --------------- |
| **1. Foundation**      | BeautyProfile, 타입, 엔진, 레지스트리 | 6       | 3 (profile, mappers, registry)   | ✅ 완료         |
| **2. Safety**          | 암호화, 동의, 4단계 파이프라인        | 7       | 3 (crypto, pipeline, repository) | ✅ 완료         |
| **3. CCS + 3엔진**     | 스코어링, skin/fashion/nutrition 엔진 | 7       | 3 (scoring, skin, fashion)       | ✅ 완료         |
| **4. Daily Capsule**   | 6단계 파이프라인, 로테이션, 컨텍스트  | 6       | 3 (daily, rotation, context)     | ✅ 완료         |
| **5. Mobile + 나머지** | 6 도메인 엔진, 쇼핑, 모바일 UI        | 24      | -                                | ✅ 완료         |
| **DB Migration**       | 7테이블 (beauty_profiles 외)          | 1       | -                                | ⏳ GFSA 후 적용 |

**구현 요약**:

- **웹 lib**: `lib/capsule/` 15모듈, `lib/safety/` 7모듈
- **웹 API**: 10 엔드포인트 (`/api/capsule/*`, `/api/safety/*`)
- **모바일**: 5 화면, 5 컴포넌트, 2 라이브러리
- **DB**: 7 테이블 SQL 작성 완료 (RLS, 인덱스, 시드 데이터)
- **테스트**: 12파일, 204개 케이스 통과
- **환경변수**: `SAFETY_ENCRYPTION_KEY` Vercel production 등록 완료

**GFSA 심사 후 할 일**:

1. `supabase db push` (7테이블 생성)
2. Vercel Ignored Build Step → "Automatic" 복원
3. 새 배포 후 캡슐 API 동작 확인

### 4.6 크로스 모듈 (신규 2026-01-30)
...
**Version**: 2.0 | **Created**: 2026-01-29 | **Updated**: 2026-03-04
**Owner**: 개발팀 / PM
**Review Cycle**: 주간
...
### v2.0 (2026-03-04) - 캡슐 에코시스템 전체 구현

- **캡슐 에코시스템 5 Phase 완료**: 60+ 파일, 204 테스트
  - Phase 1: BeautyProfile + 타입/엔진/레지스트리 (lib/capsule/ 기반)
  - Phase 2: Safety Profile — AES-256-GCM 암호화, 4단계 파이프라인
  - Phase 3: CCS 3-Layer 스코어링 + skin/fashion/nutrition 엔진
  - Phase 4: Daily Capsule 6단계 파이프라인 + 로테이션 엔진
  - Phase 5: 모바일 UI 5화면 + 나머지 6 도메인 엔진 + 쇼핑 컴패니언
- **DB 마이그레이션**: 7테이블 SQL 작성 (GFSA 심사 후 적용 예정)
- **환경변수**: SAFETY_ENCRYPTION_KEY Vercel production 등록
- **GFSA 안전 조치**: Vercel 자동 배포 비활성화, 이전 버전 롤백
- 섹션 4.5 (캡슐 에코시스템) 신규 추가
