...
> ⚠️ **2026-04-24 업데이트** — 아래 `*-v2` 엔드포인트(`skin-v2`, `body-v2`, `hair-v2`, `personal-color-v2`)는 **Orphan 상태**로 웹/모바일 UI에서 더 이상 호출되지 않습니다. 실제 사용자 분석은 **통합 플로우 `/api/analyze/integrated`** (ADR-099)에서 PC/S/C/H/M 5축을 동시 실행합니다. 개별 진입이 필요하면 v1 경로(`/api/analyze/skin` 등)를 사용하세요. v2 route는 하위 호환용 shim으로 유지되며 향후 클린업 세션에서 삭제될 수 있습니다.
