...
**체형 taxonomy**: **S/W/N(골격진단) 주(主) + ASTM 비율 보완**, 글로벌 확장 시 서양 5체형 병기.

- 5체형(hourglass/pear…) = _실루엣 비율_, S/W/N(스트레이트·웨이브·내추럴) = _골격+살집/질감_ → 같은 축의 지역 변형이 아니라 **보완 렌즈**. body-v2의 5체형을 S/W/N로 "변환" 불가(측정 대상이 다름).
- 한국 시장 우선 → S/W/N 주. body-v2의 어깨·허리·힙 비율은 *정확도/신뢰도 근거 + 비율 정보*로 보완. 글로벌 시 5체형을 추가 렌즈로 노출.
