...
**체형 taxonomy**: **S/W/N(골격진단, BodyType3) 주(主) + 측정 비율/shape 보완**, 글로벌 시 과일형(BodyShape7)/서양체형 병기.

- ⚠️ **코드 실측 (3-체계 + 매퍼 이미 존재, `lib/body/mapper.ts`)**:
  - **BodyShape7**(과일형): hourglass/pear/invertedTriangle/apple/rectangle/trapezoid/oval — C-1 비율 기반 _측정_ 결과.
  - **BodyType8**(레터형): X/A/V/H/O/I/Y/8 — 영양/운동.
  - **BodyType3**(골격): **S/W/N** — 패션/스타일링(헤어·메이크업·코디). `SHAPE7→TYPE8→TYPE3` 매핑 제공.
- 5체형/과일형(비율) = _실루엣 proportion_, S/W/N = _골격+살집/질감_ → 보완 렌즈. 매퍼가 _측정 shape → S/W/N_ 변환을 이미 담당.
- 한국 시장 우선 → 스타일링 = S/W/N 주. body-v2의 어깨·허리·힙 *측정 비율 + shape*는 정확도/신뢰도 근거로 보완(매퍼로 S/W/N 도출). 글로벌 시 과일형/서양체형 추가 노출.
- (정정: 초안의 "5체형"은 실제 BodyShape7(7 과일형). Gemini body는 별도 5형 `BodyShapeType`. 매퍼가 이 다층을 흡수.)

**인종/민족 라벨 = 지양** (입력·자동감지 모두). 민감정보(GDPR 특수범주·PIPA)·편향/PR 리스크 + 정확도엔 _측정값_(ITA°/Lab/골격 비율)이 진짜 신호라 인종 프록시 불필요. 대신 **피처 직접 측정 + 시장/로케일 기준값**(개인 인종분류가 아닌 시장 현지화)으로 정확도 확보.

**감사 결과 (통합 연결)**:

- 통합 body 축은 현재 **Gemini**(`analyzeBodyWithGemini`)를 쓰고, body-v2(MediaPipe Pose+ASTM 비율)는 표준 `/analysis/body`(`lib/body/classify.ts`)에서만 사용 → **통합 미연결**.
- ⚠️ **아키텍처 제약**: body-v2 pose 검출은 **클라이언트(브라우저 MediaPipe)**, 통합 분석은 **서버(Gemini API)** → "통합 연결"은 5줄 와이어가 아니라 _클라이언트에서 비율 산출 → API로 전달_ 설계 필요.
...
- **지금**: 체형(A) 통합 연결 감사 ✅ (결과: 통합=Gemini, body-v2=표준만, 클라이언트/서버 경계 발견) → 다음은 _클라이언트 비율 산출→API 전달_ 설계 + 멀티앵글 사다리. (pose-detector 이미 존재)
