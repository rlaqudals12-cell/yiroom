...
**모바일 패리티 (2026-06-12)**: 동일 플래그로 모바일 게이팅 완료 — 날씨 코디 2화면 redirect,
`(feed)` 레이아웃 + `(social)/feed` 3화면 가드, `/badges` 가드 + 프로필 업적 그리드 게이팅,
통합 검색의 운동/피드 결과·W/N 퀵서치 칩 게이팅. 함께 모바일 2차 표면의 ADR-098 잔존
정리: OH-1 진입점 전수 제거(뷰티 탭 카드·분석 허브·이력 탭·라우트 등록·약관 카피 —
화면 파일이 없어 눌리면 깨지던 상태), posture 진입점 제거(스타일 탭·허브) + index 가드,
캡슐 도메인 9→6(웹과 동일), 프로필 W/N 기록 섹션·목표 설정 게이팅.
