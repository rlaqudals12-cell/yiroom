...
| ------------ | ------------------------------------------------------- |
| RLS 커버리지 | 모든 테이블 100%                                        |
| 인증 방식    | 소셜 5+ 장기 목표 (Clerk 네이티브 + 검증된 커스텀 OIDC) |
| 감사 로그    | 민감 작업 100% 기록                                     |
| 권한 테스트  | 수평적 권한 상승 0건                                    |
...
## 소셜 로그인 확장 계획 (2026-03-09, 2026-08-31 정정)

> 현행 출시 범위와 도입 순서는 [ADR-121 결정 1](./ADR-121-prelaunch-product-decisions.md)을 따른다.

### 검토 중인 OAuth 프로바이더

| 프로바이더 | 설정 방식                                      | 비고                                            |
| ---------- | ---------------------------------------------- | ----------------------------------------------- |
| **카카오** | Clerk 커스텀 OIDC 프로바이더                   | Clerk 네이티브 미지원, OIDC discovery 확인      |
| **네이버** | Clerk 커스텀 OIDC 프로바이더 경로 검증 후 결정 | Clerk 네이티브 미지원, 표준 OIDC discovery 부재 |

### 구현 방식

- Google·Apple 등 Clerk 네이티브 프로바이더만 Dashboard의 Social connections 설정을 `<SignIn>` / `<SignUp>` 컴포넌트가 자동 반영한다.
- 카카오·네이버는 Social connections 토글만으로 추가할 수 없다. 카카오는 커스텀 OIDC 프로바이더로 연결하고, 네이버는 Clerk가 요구하는 OIDC 호환성을 먼저 검증한다.
- CSP의 기존 Kakao·Naver 도메인은 공유 기능·과거 배선용이며 인증 준비 완료의 근거가 아니다. 프로덕션 Clerk 도메인과 커스텀 OIDC 도입 시 CSP를 별도로 검토한다.
- 커스텀 프로바이더를 통해 Clerk 사용자가 생성된 뒤에는 기존 webhook (`user.created`)이 Supabase users 테이블을 동기화한다.
...
- **소셜 로그인 확장 경로**: Google·Apple은 Clerk 네이티브, 카카오는 커스텀 OIDC, 네이버는 OIDC 호환성 검증 후 결정
