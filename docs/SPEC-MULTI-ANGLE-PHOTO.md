# SPEC: 다각도 촬영 시스템

> 피부/퍼스널컬러 분석 정확도 향상을 위한 다각도 촬영 및 이미지 검증 시스템

**Version**: 1.0
**Date**: 2026-01-08
**Status**: Approved ✅
**Author**: Claude Code
**Implemented**: 2026-01-09

---

## 목적

전문 피부 분석기처럼 다각도 촬영을 지원하여 AI 분석 정확도를 향상시킵니다.

### 기대 효과

| 촬영 방식          | 신뢰도 | 분석 품질   |
| ------------------ | ------ | ----------- |
| 정면만             | 75%    | 기본 분석   |
| 정면 + 좌측        | 85%    | 향상된 분석 |
| 정면 + 좌측 + 우측 | 95%    | 정밀 분석   |

---

## 적용 범위

| 모듈              | 적용 여부 | 비고                           |
| ----------------- | --------- | ------------------------------ |
| S-1 (피부 분석)   | **1순위** | 피부 상태는 좌우 비대칭이 많음 |
| PC-1 (퍼스널컬러) | 2순위     | 1회성 분석, 정면만으로도 충분  |
| C-1 (체형 분석)   | 제외      | 전신 촬영으로 별도 플로우      |

---

## 기능 명세

### 1. 카메라 촬영 가이드

#### 1.1 가이드 오버레이

```
┌─────────────────────────────────┐
│                                 │
│      ╭───────────────╮          │
│      │               │          │
│      │   👤 정면     │          │  ← 얼굴 실루엣 SVG
│      │   가이드      │          │
│      │               │          │
│      ╰───────────────╯          │
│                                 │
│  "얼굴을 가이드에 맞춰주세요"    │
│                                 │
│         [ 📷 촬영 ]             │
└─────────────────────────────────┘
```

#### 1.2 각도별 가이드

| 각도        | 실루엣    | 안내 문구                    |
| ----------- | --------- | ---------------------------- |
| 정면 (필수) | 정면 얼굴 | "정면을 바라봐주세요"        |
| 좌측 (선택) | 45° 좌측  | "왼쪽으로 살짝 돌려주세요"   |
| 우측 (선택) | 45° 우측  | "오른쪽으로 살짝 돌려주세요" |

#### 1.3 UX 플로우

```
[정면 촬영] ← 필수
    ↓
"정확도를 높이려면 좌/우 사진도 추가해주세요"
    ↓
┌──────────────────────────────────┐
│                                  │
│  [좌측 추가]     [우측 추가]     │
│                                  │
│         [건너뛰기 →]             │
│                                  │
└──────────────────────────────────┘
    ↓
[분석 시작]
```

---

### 2. 갤러리 업로드 검증

#### 2.1 검증 흐름

```
[사진 선택] → [AI 검증] → 적합/부적합 판단
                  ↓
            적합: 다음 단계
            부적합: 재업로드 요청
```

#### 2.2 검증 기준

| 항목      | 기준           | 부적합 시 메시지                    |
| --------- | -------------- | ----------------------------------- |
| 얼굴 감지 | 얼굴 1개 감지  | "얼굴이 명확하게 보이지 않아요"     |
| 각도 일치 | 요청 각도 ±15° | "정면 사진이 필요해요"              |
| 조명 품질 | 적정 밝기      | "조명이 너무 어둡거나 밝아요"       |
| 메이크업  | 진한 화장 감지 | "맨 얼굴 사진이 더 정확해요" (경고) |

#### 2.3 검증 API

```typescript
// POST /api/validate/face-image
interface ValidateFaceImageRequest {
  imageBase64: string;
  expectedAngle: 'front' | 'left' | 'right';
}

interface ValidateFaceImageResponse {
  suitable: boolean;
  reason?: string; // 부적합 사유
  detectedAngle?: 'front' | 'left' | 'right' | 'unknown';
  quality: {
    lighting: 'good' | 'dark' | 'bright' | 'uneven';
    makeupDetected: boolean;
    faceDetected: boolean;
    blur: boolean;
  };
}
```

---

### 3. AI 프롬프트 (이미지 검증용)

```typescript
const validateFaceImagePrompt = `
당신은 얼굴 이미지 품질 검증 전문가입니다.

📋 검증 순서:
1. 얼굴이 이미지에 있는지 확인하세요.
2. 얼굴의 각도를 판단하세요 (정면/좌측/우측).
3. 조명 상태를 평가하세요.
4. 메이크업 여부를 확인하세요.
5. 흐림/저해상도 여부를 확인하세요.

⚠️ 판단 기준:
- 정면: 코가 얼굴 중앙, 양쪽 눈이 대칭
- 좌측: 왼쪽 얼굴이 더 많이 보임, 오른쪽 귀 안 보임
- 우측: 오른쪽 얼굴이 더 많이 보임, 왼쪽 귀 안 보임

📊 요청된 각도: ${expectedAngle}

다음 JSON 형식으로만 응답해주세요:
{
  "suitable": [true/false - 요청 각도와 일치하면 true],
  "reason": "[부적합 시 한국어 사유]",
  "detectedAngle": "[front|left|right|unknown]",
  "quality": {
    "lighting": "[good|dark|bright|uneven]",
    "makeupDetected": [true/false],
    "faceDetected": [true/false],
    "blur": [true/false]
  }
}
`;
```

---

### 4. 다각도 분석 프롬프트 (S-1 피부)

```typescript
const multiAngleSkinAnalysisPrompt = `
당신은 전문 피부과학 기반 AI 분석가입니다.

📷 제공된 이미지:
- 정면 이미지: ${hasfront ? '있음' : '없음'}
- 좌측 이미지: ${hasLeft ? '있음' : '없음'}
- 우측 이미지: ${hasRight ? '있음' : '없음'}

📋 분석 순서:
1. 각 이미지의 조명 상태를 확인하세요.
2. 정면 이미지로 전체적인 피부 상태를 평가하세요.
3. 좌측/우측 이미지가 있다면 좌우 비대칭 상태를 비교하세요.
4. 모든 이미지를 종합하여 최종 점수를 산출하세요.

⚠️ 다각도 분석 시 주의:
- 좌우 피부 상태가 다르면 더 나쁜 쪽 기준으로 점수 산출
- 조명이 다른 이미지는 신뢰도 낮춤
- 정면만 있을 경우 analysisReliability: "medium"
- 3장 모두 있을 경우 analysisReliability: "high"

📊 피부 분석 기준:
[기존 S-1 프롬프트의 분석 기준 포함]

다음 JSON 형식으로만 응답해주세요:
{
  "imagesAnalyzed": {
    "front": [true/false],
    "left": [true/false],
    "right": [true/false]
  },
  "asymmetryDetected": [true/false],
  "asymmetryDetails": "[좌우 비대칭 상세 - 있을 경우]",
  // ... 기존 S-1 결과 필드들
  "analysisReliability": "[high|medium|low]"
}
`;
```

---

## 데이터베이스

### 스키마 변경

```sql
-- skin_analyses 테이블에 다각도 이미지 컬럼 추가
ALTER TABLE skin_analyses
  ADD COLUMN IF NOT EXISTS front_image_url TEXT,
  ADD COLUMN IF NOT EXISTS left_image_url TEXT,
  ADD COLUMN IF NOT EXISTS right_image_url TEXT,
  ADD COLUMN IF NOT EXISTS images_count INT DEFAULT 1;

-- 기존 image_url을 front_image_url로 마이그레이션
UPDATE skin_analyses
SET front_image_url = image_url
WHERE front_image_url IS NULL AND image_url IS NOT NULL;
```

---

## 컴포넌트 구조

```
components/analysis/
├── camera/
│   ├── FaceGuideOverlay.tsx      # 얼굴 가이드 오버레이
│   ├── AngleSelector.tsx         # 각도 선택 UI
│   └── MultiAngleCapture.tsx     # 다각도 촬영 플로우
├── validation/
│   └── ImageValidator.tsx        # 이미지 검증 결과 표시
└── skin/
    └── MultiAngleSkinCapture.tsx # S-1용 통합 컴포넌트
```

---

## API 엔드포인트

| 메서드 | 경로                       | 설명                    |
| ------ | -------------------------- | ----------------------- |
| POST   | `/api/validate/face-image` | 얼굴 이미지 검증        |
| POST   | `/api/analyze/skin`        | 피부 분석 (다각도 지원) |

### 요청 형식 변경

```typescript
// 기존
interface SkinAnalysisRequest {
  imageBase64: string;
}

// 변경
interface SkinAnalysisRequest {
  frontImageBase64: string; // 필수
  leftImageBase64?: string; // 선택
  rightImageBase64?: string; // 선택
}
```

---

## 비용 분석

### Gemini API 토큰

| 항목      | 1장  | 3장    | 증가율   |
| --------- | ---- | ------ | -------- |
| 입력 토큰 | ~300 | ~800   | +167%    |
| 출력 토큰 | ~500 | ~500   | 0%       |
| **총합**  | ~800 | ~1,300 | **+63%** |

### 검증 API 추가 비용

- 이미지당 검증: ~200 토큰
- 3장 검증 시: ~600 토큰 추가
- **총 3장 분석 비용**: ~1,900 토큰 (기존 대비 2.4배)

> Gemini 3 Flash 가격이 저렴하므로 수용 가능한 수준

---

## 구현 순서

### Phase 1: 이미지 검증 API

1. `/api/validate/face-image` 엔드포인트 구현
2. 검증 프롬프트 작성 및 테스트
3. Mock fallback 구현

### Phase 2: 카메라 가이드 컴포넌트

1. `FaceGuideOverlay` 컴포넌트 (SVG 가이드)
2. `AngleSelector` 컴포넌트 (좌/우 선택)
3. `MultiAngleCapture` 통합 플로우

### Phase 3: S-1 통합

1. `skin_analyses` 테이블 마이그레이션
2. S-1 API 다각도 지원 추가
3. S-1 프롬프트 업데이트
4. 기존 테스트 업데이트

### Phase 4: 갤러리 업로드 검증

1. 업로드 시 자동 검증 연동
2. 부적합 시 재업로드 UI

---

## 테스트 케이스

### 검증 API

- [x] 정면 사진 → 정면 요청 시 suitable: true
- [x] 좌측 사진 → 정면 요청 시 suitable: false
- [x] 얼굴 없는 사진 → faceDetected: false
- [x] 어두운 사진 → lighting: "dark"

### 다각도 분석

- [x] 정면만 분석 → analysisReliability: "medium"
- [x] 3장 분석 → analysisReliability: "high"
- [x] 좌우 비대칭 감지 → asymmetryDetected: true

---

## 참고

- [prompt-engineering.md](../.claude/rules/prompt-engineering.md) - 프롬프트 작성 규칙
- [ai-integration.md](../.claude/rules/ai-integration.md) - AI 통합 패턴
- [SDD-VISUAL-ANALYSIS-ENGINE.md](./SDD-VISUAL-ANALYSIS-ENGINE.md) - 비주얼 분석 엔진

---

**Approved by**: Claude Code
**Implementation Start**: 2026-01-09
**Implementation Complete**: 2026-01-09

---

## 구현 결과

### 구현된 파일

```yaml
Phase 1: 이미지 검증 API ✅
- types/visual-analysis.ts (FaceAngle, ValidateFaceImageRequest/Response, MultiAngleImages)
- lib/mock/face-validation.ts (Mock 생성기)
- lib/gemini.ts (validateFaceImage 함수)
- app/api/validate/face-image/route.ts (API 엔드포인트)
- tests/api/validate/face-image.test.ts (13 테스트)

Phase 2: 카메라 가이드 컴포넌트 ✅
- components/analysis/camera/FaceGuideOverlay.tsx (SVG 가이드)
- components/analysis/camera/AngleSelector.tsx (각도 선택 UI)
- components/analysis/camera/MultiAngleCapture.tsx (통합 플로우)
- components/analysis/camera/index.ts (exports)
- tests/components/analysis/camera/*.test.tsx (19 테스트)

Phase 3: S-1 통합 ✅
- app/(main)/analysis/skin/_components/MultiAngleSkinCapture.tsx (S-1 래퍼)
- app/api/analyze/skin/route.ts (다각도 지원 추가)
- tests/api/analyze/skin.test.ts (업데이트)
```

### 테스트 결과

- 총 44개 테스트 통과
  - 검증 API: 13개
  - 카메라 컴포넌트: 19개
  - 피부 분석 API: 12개
