...
function classifySeason(undertone: 'warm' | 'cool', L: number, C: number): Season {
  if (undertone === 'warm') {
    // 밝고 선명 = 봄, 어둡고 뮤트 = 가을
...
function classifyTwelveTone(undertone: 'warm' | 'cool', L: number, C: number): TwelveToneResult {
...
function bradfordAdapt(
  ...
  // 스케일 행렬
  const scale = [tgtLMS[0] / srcLMS[0], tgtLMS[1] / srcLMS[1], tgtLMS[2] / srcLMS[2]];

  // 적용
  const lms = multiplyMatrix(M, xyz);
  const adaptedLMS = [lms[0] * scale[0], lms[1] * scale[1], lms[2] * scale[2]];
...
function calculateCIEDE2000(
  ...
  const deltaE00 = Math.sqrt(
    dLPrimeKLSL ** 2 + dCPrimeKCSC ** 2 + dHPrimeKHSH ** 2 + RT * dCPrimeKCSC * dHPrimeKHSH
  );
...
```typescript
// 파운데이션은 피부톤과 직접 매칭
function matchFoundation(skinLab: LabColor, products: FoundationProduct[]): FoundationMatch[] {
  return products
    .map((product) => {
      const productLab = hexToLab(product.hex);
      const deltaE = calculateCIEDE2000(skinLab, productLab);
...
- **디지털 드레이핑**: 전문가 40+ drape 비교를 사진에 색 오버레이→시프트 시각화로 대체 (→ [draping-simulation.md](./draping-simulation.md)).
- **다중 undertone 신호**: gold/silver 테스트 + 손목 혈관(통합 플로우에 손목 사진 옵션) + Lab(PC-2). undertone(warm/cool)이 최난도라 단일 신호 의존 금지.
- **자연광/화이트밸런스 엄격 검증**: 단일 셀카 정확도는 *장수*가 아니라 *조명*이 변수 → CIE-1 품질 게이트 표면화 (자연광·노필터·노메이크업).
- 출처: [IED 색채학](https://www.ied.edu/news/armocromy-science-colours-personal-styling); [Style Academy](https://www.styleacademyintl.com/post/mastering-seasonal-color-analysis).
- 상세 → [ADR-108](../adr/ADR-108-axis-accuracy-upgrade-roadmap.md)

---

**Version**: 1.4 | **Created**: 2026-01-16 | **Updated**: 2026-06-17
