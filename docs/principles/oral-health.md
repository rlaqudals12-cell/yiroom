# 구강건강 원리 — [SUPERSEDED 2026-04-22]

> ⚠️ **이 문서는 [ADR-098 정체성 재정의 v2](../adr/ADR-098-identity-redefinition-5axis-model.md)에 의해 폐기되었습니다.**
>
> OH-1(구강건강) 모듈은 2026-04-22 완전 제거되었습니다. 이 원리 문서는 역사 보존 목적으로만 남겨둡니다.
>
> **제거 근거**:
>
> - 이룸의 "시각 정체성 5축"(PC/S/C/H/M) 포지셔닝과 도메인 이질
> - "거울 + 옷장 이해" 정체성 맥락에서 빠지는 영역
...
> **관련 문서**:
>
> - [ADR-098](../adr/ADR-098-identity-redefinition-5axis-model.md) — 정체성 재정의
> - [ULTIMATE-FORM.md §0](../ULTIMATE-FORM.md) — 5축 모델
> - 제거된 코드 커밋: `106ddd59` (2026-04-22)

---

## 0. 궁극의 형태 (P1) — [v1, 폐기]

> 아래 내용은 v1 정의 기록 보존용. 실제 적용되지 않음.
...
### 2.3 VITA 3D-Master 29색 체계

VITA 3D-Master는 Classical의 16색 한계를 극복하기 위해 3차원 색공간(밝기 × 채도 × 색상)을 기반으로 설계된 체계다.

**구조: Value Group × Chroma**

|   Value Group   | 밝기(L\*) 범위 | 셰이드 수 | 셰이드 목록                               |
| :-------------: | :------------: | :-------: | ----------------------------------------- |
|  0 (Bleached)   |     80-90      |     3     | 0M1, 0M2, 0M3                             |
|    1 (밝음)     |     68-73      |     5     | 1M1, 1M2, 1M2.5 + ★                       |
|  2 (중간-밝음)  |     63-69      |     7     | 2L1.5, 2L2.5, 2M1, 2M2, 2M3, 2R1.5, 2R2.5 |
|    3 (중간)     |     58-64      |     7     | 3L1.5, 3L2.5, 3M1, 3M2, 3M3, 3R1.5, 3R2.5 |
| 4 (중간-어두움) |     52-58      |     4     | 4L1.5, 4M1, 4M2, 4R1.5                    |
|   5 (어두움)    |     45-52      |     3     | 5M1, 5M2, 5M3                             |
|    **합계**     |                |  **29**   |                                           |

★ Value Group 1에는 추가로 1M1이 L/R 변형 없이 단독 존재

**Chroma 분류**:

- **L** (Low): 낮은 채도 — 회색기 강함, 낮은 a*·b*
- **M** (Medium): 중간 채도 — 표준 자연 치아색
- **R** (Rich): 높은 채도 — 붉은기/노란기 강함, 높은 a*·b*

**3D-Master Lab 참조값** (대표 셰이드):

| 셰이드 | L\*  | a\* | b\* | Classical 대응 |
| :----: | :--: | :-: | :-: | :------------: |
|  0M1   |  88  |  0  |  8  |     (없음)     |
|  0M2   |  85  | 0.5 | 10  |     (없음)     |
|  0M3   |  82  |  1  | 12  |     (없음)     |
...
- 모든 3D-Master 셰이드는 가장 근접한 Classical 셰이드와 매핑됨
- 3D-Master가 더 정밀한 분류를 제공 (16 → 29 셰이드)
- Bleached 3종(0M1-0M3)은 Classical에 대응 없음

**3D-Master 매칭 알고리즘**:

```typescript
function findBest3DShadeMatch(
  measuredLab: LabColor,
  excludeBleached?: boolean
): {
  shade: Vita3DMasterShade | VitaBleachedShade;
  deltaE: number;
  classicalEquivalent: VitaClassicalShade;
  alternatives: Array<{ shade: string; deltaE: number }>;
};
```

- CIEDE2000 색차로 29개 참조값과 비교
- `excludeBleached` 옵션으로 블리치드 셰이드 제외 가능
- 상위 3개 대안 셰이드도 함께 반환

### 2.4 변색 원인별 미백 목표 세분화

치아 변색은 원인에 따라 미백 가능성이 크게 달라진다:

| 원인                         | 설명                            | 자가 미백 효과 | 전문 시술 필요 |
| ---------------------------- | ------------------------------- | :------------: | :------------: |
| **표면 착색** (surface)      | 커피, 와인, 흡연 등 외부 요인   |      90%       |       ❌       |
| **노화 변색** (aging)        | 법랑질 마모, 상아질 노출        |      60%       |       ❌       |
| **내재적 변색** (intrinsic)  | 치아 내부 구조 (외상, 과불소증) |      40%       |       ✅       |
| **항생제 변색** (antibiotic) | 테트라사이클린 등 약물          |      20%       |       ✅       |

**원인별 추천 경로 분기**:

- `surface`: 미백 치약 → 스트립 → 전문 미백 (순차적)
- `aging`: 저자극 미백 치약 → 전문 미백 (법랑질 상태 고려)
- `intrinsic`: 인-오피스 미백 → 전문 블리칭 (자가 미백 한계)
- `antibiotic`: 인-오피스 블리칭 전용 (자가 미백 거의 무효)

**기간 조정 공식**:

```
adjustedDuration = baseDuration × (1 / selfBleachingEffectiveness)
```

효과가 낮을수록 더 오래 걸림 (최대 3배 제한).
...
| 초기 집락       | 0-4시간       | _S. mitis_, _S. oralis_ 부착 |
| 바이오필름 성숙 | 4-24시간      | _F. nucleatum_ 가교          |
...
**Version**: 1.1 | **Created**: 2026-01-17 | **Updated**: 2026-03-11
