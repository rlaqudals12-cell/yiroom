...
### KI-007: 홈 화면 정보 블록 7개 초과 — **부분 해결**

- **발견 횟수**: 2회 (웹 홈 초기 발견, 모바일 홈 3/28 재발견)
- **관련 항목**: F1
...
### KI-011: 성별-스타일 용어 불일치 (D4+D9 복합) — **해결됨**

- **발견 횟수**: 3회 (모바일 types.ts, 웹 gender/page.tsx, 웹 GenderSelector.tsx)
- **관련 항목**: D4 (신체 부정), D9 (자율성), A1 (용어 일관성)
- **발생 화면**: 온보딩 Step 2 (모바일 + 웹)
- **원인**: 성별 선택 후 "남성적/여성적 스타일" 라벨이 성별 이분법 강화. 또한 웹에서 동일 개념에 3가지 다른 라벨("남성적 스타일" / "매스큘린" / "masculine") 사용
- **수정 방법**: 성별 중립 라벨로 변경 (미니멀/소프트/자유로운), 스타일 설명 해요체 통일, @yiroom/shared에 상수 추출
- **재발 방지**: `packages/shared/src/onboarding/` Single Source of Truth. 새 라벨 추가 시 shared에서만 수정
- **해결 커밋**: 2026-03-28 모바일+웹 10개 파일 일괄 수정
