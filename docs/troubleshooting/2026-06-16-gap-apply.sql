-- ============================================================
-- 갭 적용 스크립트 (일회성) — 복원된 prod DB ihlrjhqyopjmhqvjnnnu 전용
-- 생성: 2026-06-16 by Claude Code (rev2: auth.get_user_id 함수 제거 → auth.jwt()->>'sub' 인라인)
-- 목적: 복원 DB(~Feb12 + 수동 20260423)에 빠진 8개 테이블 + persona 컬럼 보강
--   누락분: capsules/capsule_items/daily_capsules/rotation_history/beauty_profiles/
--           cross_domain_rules/safety_profiles/user_shopping_preferences
--           + integrated_analysis_sessions.persona (20260424)
-- 적용: Supabase Dashboard > SQL Editor 에 전체 붙여넣기 1회 Run.
--   전체가 트랜잭션 → 실패 시 자동 롤백(부분적용 없음). 재실행 시 정책중복→롤백(무해).
--   rev2: auth 스키마 함수 생성 권한 없음(42501) 회피 — 모든 RLS는 auth.jwt()->>'sub' 인라인.
-- ============================================================

BEGIN;

-- [1/4] 캡슐 기반 6테이블 — 20260304_capsule_foundation.sql
...
-- [2/4] safety_profiles — 20260304_safety_profiles.sql (auth.jwt()->>'sub')
...
-- [3/4] user_shopping_preferences — 202512290300_phase_j_smart_matching.sql 발췌
...
-- [4/4] integrated_analysis_sessions.persona 컬럼 — 20260424_persona_profile_column.sql
