# 트러블슈팅 문서 인덱스

> 개발 중 발생한 이슈와 해결 방법 기록

---

## 문서 목록

| 날짜       | 제목                                                                                                    | 영향 영역         | 상태       |
| ---------- | ------------------------------------------------------------------------------------------------------- | ----------------- | ---------- |
| 2026-03-08 | [홈 화면 UX 정제 — 반복 점검 기반 개선](./2026-03-08-home-ux-refinement.md)                             | 홈 UX, 5 컴포넌트 | 해결됨     |
| 2026-03-07 | [Clerk Error 1016 — Origin DNS Error](./2026-03-07-clerk-error-1016-origin-dns.md)                      | Clerk 인증, DNS   | ⏳ 대기 중 |
| 2026-03-07 | [Capsule API Slug 충돌 — domain vs id](./2026-03-07-capsule-api-slug-conflict.md)                       | API 라우팅        | ✅ 해결됨  |
| 2026-03-04 | [캡슐 에코시스템 배포 — GFSA 심사 중 자동 배포 사고 방지](./2026-03-04-capsule-ecosystem-deployment.md) | Vercel 배포, DB   | ✅ 해결됨  |
| 2026-03-03 | [분석 API DB 저장 실패 시 응답 차단](./2026-03-03-analysis-api-db-resilience.md)                        | 분석 API 11개     | ✅ 해결됨  |
| 2026-02-20 | [Expo Go 에뮬레이터 실행](./2026-02-20-expo-go-emulator-startup.md)                                     | 모바일 개발환경   | ✅ 해결됨  |
| 2026-02-18 | [Expo Router URL 충돌](./2026-02-18-expo-router-url-collision.md)                                       | 모바일 라우팅     | ✅ 해결됨  |
| 2026-02-10 | [C-1 갤러리 업로드 버그 + 4각도 확장](./2026-02-10-body-multi-angle-gallery-bug.md)                     | 체형 분석, DB     | ✅ 해결됨  |
| 2026-02-09 | [레이아웃 깨짐 조사](./2026-02-09-layout-collapse-investigation.md)                                     | 전체 페이지       | ✅ 해결됨  |
| 2026-02-05 | [퍼스널컬러 분석 UX 이슈](./2026-02-05-personal-color-ux-issues.md)                                     | UI, API           | ✅ 해결됨  |

---

## 카테고리별 분류

### 레이아웃/렌더링 이슈

- [2026-02-09] 전체 페이지 레이아웃 축소 (home 제외) — `@theme inline` spacing 순환 참조

### 카메라/업로드 이슈

- [2026-02-10] C-1 갤러리 업로드 버그 — `handleFileSelect`에서 측면/후면 UI 갱신 누락

### UI/UX 이슈

- [2026-03-08] 홈 대시보드 UX 정제 — `/ux-check` 반복 점검으로 67% → 97% 개선 (7개 항목 수정)
- [2026-02-05] 좌/우 사진 카드 크기 및 정렬 문제

### 모바일 개발 환경 이슈

- [2026-02-20] Expo Go 에뮬레이터 실행 — CLI 버그, SDK 불일치, 네트워크 설정 3중 문제
  - 빠른 실행: `bash scripts/expo-emulator.sh`

### 모바일 라우팅 이슈

- [2026-02-18] Expo Router 라우트 그룹 URL `/` 충돌 — 알파벳순으로 `(challenges)`가 기본 화면

### 배포/인프라 이슈

- [2026-03-07] Clerk 개발 인스턴스 Cloudflare Worker 530 에러 — Clerk 인프라 측 Origin DNS 장애
  - 로컬 DNS(SK Broadband -> Google) 변경으로 해결 안 됨, Clerk 복구 대기
- [2026-03-04] GFSA 심사 중 자동 배포 트리거 — Vercel 롤백 + Ignored Build Step "Don't build anything"
  - GFSA 후 복원 체크리스트 포함

### API/환경 설정 이슈

- [2026-03-03] 분석 API DB 저장 실패 시 응답 차단 — 11개 라우트 `if (usedMock)` 가드 제거, 무조건 합성 응답
  - 부가: Vercel 환경변수 `\n` 오염, 빌드 캐시 파일 불일치 (`--force` 해결)
- [2026-02-05] AI 분석 실패 - Mock Fallback 비활성화

---

## 문서 작성 가이드

### 파일명 규칙

```
YYYY-MM-DD-이슈-제목.md
```

### 템플릿

```markdown
# [이슈 제목]

> **날짜**: YYYY-MM-DD
> **영향 파일**: 파일 경로
> **심각도**: 높음/중간/낮음
> **상태**: ✅ 해결됨 / ⏳ 진행 중 / ❌ 미해결

---

## 1. [이슈 제목]

### 증상

[무엇이 잘못되었는지]

### 원인

[왜 발생했는지]

### 해결

[어떻게 해결했는지]

### 영향 파일

- 파일 경로

### 교훈

[향후 예방법]
```

---

**Updated**: 2026-03-08

### 라우팅 충돌 이슈

- [2026-03-07] Next.js App Router dynamic slug 충돌 — `[domain]` vs `[id]` 동일 레벨 금지
  - 해결: `[id]/check/` → `check/[id]/` 재구성
