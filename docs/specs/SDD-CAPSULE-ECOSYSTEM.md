...
> **배치 R 보완 (2026-09-08)**: 캡슐 아이템 생성·단건 추가·로테이션의
> `clerk_user_id`는 부모 캡슐 소유자와 일치해야 한다. 단건 추가는 부모 조회 실패 시
> 삽입하지 않는다. 대시보드 `/capsule`·`/capsule/[domain]`·`/capsule/gap` 3페이지는
> `(retired)` 라우트 그룹 레이아웃에서 `/home`으로 돌려 출시 전 노출을 막는다.
> `/capsule/daily`는 ADR-111 정본 표면(홈 위젯 "전체 루틴 보기"의 목적지)이라 유지한다.
