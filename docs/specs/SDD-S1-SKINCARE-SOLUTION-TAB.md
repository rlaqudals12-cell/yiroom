...
## 배치 S 안전·근거 계약 (2026-09-08)

- 근거: 외부 보고서 `2026-09-07-skincare-consultation-sim.md` D-2/D-3의 공식 출처와 50개 판정. 신규 조사 없이 기존 `skin-physiology.md`와 ADR-117의 결정론·정직성 원칙을 적용한다.
- 사용법의 각 문장은 claim ID, evidence(established/conventional/unsupported/hygiene), 출처·적용 범위를 가진다. 기존 50개 판정은 감사 목록으로 보존하고 unsupported 원문은 렌더하지 않는다. 재작성은 효능·정밀 수치 없이 별도 승인 ID로 제공한다.
- 사용법과 기존 tips 모두 승인 목록으로 렌더한다. 로케일은 동일 ID/등급/조건을 유지한다. 코치는 승인 목록만 선택하며 숫자·출처·등급을 생성하지 않는다.
- `getStepSpec` 및 제품 배치/카탈로그/12존 추천에 안전 문진과 시간대를 전달한다. 누락은 미문진으로 취급하고 일반 세안·보습·자외선 차단 안내만 제공한다. 임신·수유·이소트레티노인 및 오전에는 레티노이드 후보를 제외한다.
- 카탈로그 성분 누락은 안전성 확인으로 간주하지 않는다. 제한 상태에서 확인 불가능한 신규 제품은 제안하지 않는다. 제품별 사용량·빈도·도포 부위는 라벨을 우선한다.
- 검증: 50개 원판정 보존, 가시 unsupported 0, 수치 출처 검사, 4언어 동일 ID, 임신/수유/복용/미문진 및 오전의 스펙·제품·12존 경로 회귀 테스트.
