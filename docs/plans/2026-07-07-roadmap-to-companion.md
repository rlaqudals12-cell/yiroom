...
- [~] 어필리에이트 전환 실측 → 수익 모델 검증 — **클릭 실측 인프라 개통 ✅(5af02273, 2026-07-08)**: prod 실사로 상류 전멸 발견·수리(구매 링크 0/700→700 검색 링크 채움, affiliate_clicks 스키마 정합 #17, source_page 귀속 배선, anon insert 스모크 통과). **잔여 = 파트너십 체결(사용자: 쿠팡 파트너스/올리브영 등) → affiliate_url 전환 → 전환 웹훅 실연동**. 검색 링크는 무수수료지만 클릭 수요 실측 = 파트너십 피칭 데이터
