...
## 자율 범위와 완료 기준 (한 배치 = 한 세션)

- **자율 범위**: 배치 프롬프트의 항목 전부를 **중간 질문·승인 요청 없이** 끝까지 수리한다.
  판단이 갈리는 지점은 멈추지 말고 가장 보수적인 안으로 진행하고, 그 질문을 **최종 보고서에
  기재**한다.
- **완료 기준**: 각 항목이 테스트로 증명되고 아래 Verify-Loop가 전그린이면 완료. 완료 시
  항목별 원인·수리·근거 `파일:라인`·검증 수치를 담은 보고서 **1회** — 이 보고서가 유일한 산출물이다.
- **커밋·push는 하지 않는다**(아래 하드 룰). 검증·커밋은 Claude Code(Fable)가 보고서를 받아 수행한다.

## 보고서 저장 규약 (채팅 붙여넣기 대신 파일)

- 모든 최종 보고서(코드 배치·리서치·시뮬레이션)는 **`c:/tmp/yiroom-codex-reports/YYYY-MM-DD-<slug>.md`**
  에 저장한다(리포 밖 — 커밋 대상 아님). 같은 날 같은 주제면 `-v2`, `-v3` 접미사.
- 채팅 마지막 줄에는 **저장 경로 한 줄만** 출력한다. 창업자가 "최신 보고서 확인해줘"라고 하면
  Claude Code가 이 폴더의 최신 파일을 읽는다.
- 보고서 형식: 맨 위에 `# 제목`, `> 배치/과제명 · 날짜 · 모델`, 그 아래 항목별 원인→수리·근거
  `파일:라인`→검증 수치. 커밋하지 않았음을 명시.
